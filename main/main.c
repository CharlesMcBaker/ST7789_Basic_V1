...
// MOSI		Yellow	GPIO number		23
// SCLK		Orange	GPIO number		18
// CS		Purple	GPIO number		5
// DC		Blue	GPIO number		27
// RESET	Green	GPIO number		33
// BL		White	GPIO number		32
...
TickType_t CharlieTest(TFT_t * dev, FontxFile *fx, int width, int height) 
{
	TickType_t startTick, endTick, diffTick;
	startTick = xTaskGetTickCount();
	...
	color = RED;
	strcpy((char *)ascii, "5 BAR");
	lcdSetFontDirection(dev, 0);
	lcdDrawString(dev, fx, 0, fontHeight-1, ascii, color);

	color = GREEN;
	strcpy((char *)ascii, "10 BAR");
	lcdSetFontDirection(dev, 3);
	lcdDrawString(dev, fx, (fontHeight-1), height-1, ascii, color);
...
void ST7789_Custom(void *pvParameters)
{
	FontxFile fx16G[2];
	FontxFile fx24G[2];
	FontxFile fx32G[2];
	...
	TFT_t dev;
	spi_master_init(&dev, CONFIG_MOSI_GPIO, CONFIG_SCLK_GPIO, CONFIG_CS_GPIO, CONFIG_DC_GPIO, CONFIG_RESET_GPIO, CONFIG_BL_GPIO);
	lcdInit(&dev, CONFIG_WIDTH, CONFIG_HEIGHT, CONFIG_OFFSETX, 18);
...
void bmp280_test(void *pvParameters)
{
    ...
    ESP_ERROR_CHECK(bmp280_init_desc(&dev, BMP280_I2C_ADDRESS_0, 0, 17, 18));
...
void app_main(void)
{
	...
	xTaskCreate(ST7789_Custom, "ST7789_Custom", 1024*6, NULL, 2, NULL);
